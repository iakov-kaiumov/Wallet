...
final class HeaderView: UIView {
    ...
    func configure(model: PersonModel) {
        balanceLabel.setText(model.personBalance?.displayString(currency: .RUB) ?? "")
        incomeLabel.setText(model.personIncome?.displayString(currency: .RUB) ?? "")
        expencesLabel.setText(model.personSpendings?.displayString(currency: .RUB) ?? "")
